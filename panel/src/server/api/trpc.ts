...
const clientGuardMiddleware = t.middleware(async ({ ctx, next }) => {
    ...
    if (ctx.session.user.role !== 'CLIENT' || ctx.session.user.clientId == null) {
        throw new TRPCError({ code: 'FORBIDDEN', message: 'Forbidden' });
    }

    const client = await ctx.db.clients.findUnique({
        where: { id: ctx.session.user.clientId },
    ...
    if (!client || !client.status) {
        throw new TRPCError({
            code: 'UNAUTHORIZED',
            message: 'Client no longer exists or is disabled',
        });
    }
    ...
    return next({
        ctx: {
            session: { ...ctx.session, user: ctx.session.user },
            clientId: client.id,
            clientServerId: client.serverId,
        },
    });
});

export const clientProcedure = t.procedure.use(timingMiddleware).use(clientGuardMiddleware);
